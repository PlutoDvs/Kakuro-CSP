...
class Kakuro:
    def __init__(self, board, filtering=None,
        ...
        self.state = board
        self.unassigned_variables = self.variables.copy()
        self.domains = self.raw_domains
    ...
    def assign(self, var, value):
        ...
        self.unassigned_variables.remove((var[0], var[1]))
...
def backtrack(kaku):
    global counter
    # time.sleep(0.3)
    if len(kaku.curr_assignments) == len(kaku.variables):
        return kaku.curr_assignments

    var = kaku.select_unassigned_variable()
    orders = kaku.order_domain_values(var)

    for value in orders:
        if kaku.is_consistent(var, value):
            pre_csp = copy.deepcopy(kaku)
            pre_csp.state = kaku.state
            kaku.assign(var, value)
            counter += 1
            if kaku.filter_domain_values(var):
                result = backtrack(kaku)
                if result is not None:
                    return result
            kaku = pre_csp
            kaku.state[var[0]][var[1]] = ''
    return None
